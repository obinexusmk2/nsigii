...
const CLASS = ["NOISE", "NONOISE", "SIGNAL", "NOSIGNAL"];
const CONSENSUS = { 0xff: "YES", 0x00: "NO", 0x10: "MAYBE" };
const dec = new TextDecoder("latin1");
const asciiTrim = (b, off, len) => dec.decode(b.subarray(off, off + len)).replace(/\0+$/g, "").replace(/\0/g, "");
...
function u64(dv, off) {
    ...
    if (v > BigInt(Number.MAX_SAFE_INTEGER))
        throw new Error(`NSIGII offset too large at ${off}`);
    return Number(v);
}
/** Parse the wrapper structure. Throws on a bad magic or out-of-bounds offsets. */
export function inspectBytes(input) {
    const b = toU8(input);
    const dv = new DataView(b.buffer, b.byteOffset, b.byteLength);
    if (asciiTrim(b, 0, 6) !== "NSIGII" || b[6] !== 0x00)
        throw new Error("Not a constitutional wrapper");
    let footerPos = lastIndexOf(b, FOOTER_MAGIC);
    let footerLen = FOOTER_MAGIC.length;
    if (footerPos < 0) {
        footerPos = lastIndexOf(b, FOOTER_MAGIC_LEGACY);
        footerLen = FOOTER_MAGIC_LEGACY.length;
    }
    if (footerPos < 0)
        throw new Error("NSIGII footer not found");
    ...
    for (const [n, o] of [["channelTableOffset", channelTableOffset], ["segmentTableOffset", segmentTableOffset], ["payloadOffset", payloadOffset]]) {
        if (!Number.isSafeInteger(o) || o < 0 || o >= b.length)
            throw new Error(`Invalid NSIGII ${n}: ${o}`);
    }
    if (payloadOffset + payloadSize > b.length)
        throw new Error("Invalid NSIGII payload bounds");
    ...
    for (let i = 0; i < channelCount; i++) {
        const id = b[off++];
        const roleLen = b[off++];
        const role = asciiTrim(b, off, roleLen);
        off += roleLen;
        const hash = asciiTrim(b, off, HASH_HEX);
        off += HASH_HEX;
    ...
    for (let i = 0; i < TRIDENT; i++) {
        const segmentId = u64(dv, off);
        off += 8;
        const channelId = b[off++];
        off += 8; // timestampNs
        const segPayloadOffset = u64(dv, off);
        off += 8;
        const segPayloadSize = u64(dv, off);
        off += 8;
        const segPayloadHash = asciiTrim(b, off, HASH_HEX);
        off += HASH_HEX;
    ...
    const vSegmentId = u64(dv, off);
    off += 8;
    const transmitHash = asciiTrim(b, off, HASH_HEX);
    off += HASH_HEX;
    const receiveHash = asciiTrim(b, off, HASH_HEX);
    off += HASH_HEX;
    const verifyHash = asciiTrim(b, off, HASH_HEX);
    off += HASH_HEX;
    const consensus = CONSENSUS[b[off++]] ?? "MAYBE";
    const consensusScore = dv.getFloat32(off, true);
    off += 4;
...
export async function verifyBytes(input) {
    const b = toU8(input);
    ...
    const roles = ["TRANSMIT", "RECEIVE", "VERIFY"];
...
export async function extractBytes(input) {
    const b = toU8(input);
...
//# sourceMappingURL=constitutional.mjs.map