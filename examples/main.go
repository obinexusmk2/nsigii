...
func (tc *TridentChannel) EncodeMessage(rawContent []byte) TridentPacket {
	...
	// SHA-256 of the ROPEN-encoded payload; the receiver recomputes and compares.
	packet.Payload.MessageHash = sha256.Sum256(encoded)
...
func (tc *TridentChannel) DecodePacket(packet TridentPacket) TridentPacket {
	// Verify hash integrity: recompute SHA-256 of the received payload.
	if sha256.Sum256(packet.Payload.Content) != packet.Payload.MessageHash {
...
func (nc *NSIGIICodec) EncodeFrame(frame []byte) ([]byte, error) {
	...
	// --- Trident verification side-channel --------------------------------
	// The three channels derive a per-frame verdict (ORDER / CHAOS / VERIFIED)
	// and the RB-AVL confidence state. They DO NOT carry the pixels: ROPEN is a
	// non-invertible 2->1 digest (see ropen.c / docs "What's still broken").
	packet := nc.Channels[CHANNEL_TRANSMITTER].EncodeMessage(unified)
	packet = nc.Channels[CHANNEL_RECEIVER].DecodePacket(packet)
	nc.Channels[CHANNEL_VERIFIER].VerifyPacket(packet) // updates channel State

	// --- Pixel path ------------------------------------------------------------
	// Convert the ORIGINAL, full-length RGB24 frame to I420. Feeding the halved
	// ROPEN digest here is the "image doubled over a solid-green lower half" bug
	// from docs/File without any code.txt ("The bug that three renderers found").
	yuvFrame := nc.rgbToYUV420(frame)
...
func main() {
	var width, height, maxFrames int
	...
	flag.IntVar(&maxFrames, "frames", 0, "encode at most N frames (0 = all)")
	...
	for {
		if maxFrames > 0 && frameCount >= maxFrames {
			log.Printf("Reached -frames %d cap", maxFrames)
			break
		}
